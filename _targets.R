...
inputs <- list(geog = list(tar_files(infile_geog, "~/../cloudstor/Shared/Environment_General//ABS_data/ABS_meshblocks/abs_meshblocks_2016_data_provided/MB_2016_WA.shp", 
    ...
    exposure = list(tar_files(infile_exposure, c("~/../cloudstor/Shared/Environment_General//Air_pollution_model_GlobalGWR_PM25/GlobalGWR_PM25_V4GL02/data_derived/GlobalGWR_PM25_GL_201501_201512-RH35-NoNegs_AUS_20180618.tif", 
    "~/../cloudstor/Shared/Environment_General//Air_pollution_model_GlobalGWR_PM25/GlobalGWR_PM25_V4GL02/data_derived/GlobalGWR_PM25_GL_201401_201412-RH35-NoNegs_AUS_20180618.tif"
    ), format = "file"), tar_target(tidy_data_exposure_part, 
        tidy_exposure(infile_exposure), pattern = map(infile_exposure), 
        iteration = "list")), impact_pop = list(tar_files(infile_impact_pop, 
...
derive_data <- list(tar_target(data_study_pop_health, do_study_pop_impact(tidy_data_study_pop, 
    tidy_data_impact_pop)), tar_target(data_env_exposure, do_env_exposure(tidy_data_exposure_part, 
    tidy_data_geog), pattern = map(tidy_data_exposure_part), 
    iteration = "list"), tar_target(data_env_counterfactual, 
    do_env_counterfactual(data_env_exposure, "min"), pattern = map(data_env_exposure), 
    iteration = "list"), tar_target(combined_exposures_pop, rbindlist(data_env_counterfactual)), 
    tar_target(data_linked_pop_health_enviro, do_linked_pop_health_enviro(data_study_pop_health, 
        combined_exposures_pop, tidy_data_exp_pop)))
